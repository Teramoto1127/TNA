...
## 🔐 ログイン・権限管理 (New!)
本バージョンより、ログインロール（役割）による機能制限と画面出し分けを追加しました。
- **トップページ (Index)**: サービス概要の紹介と、一般/ホストの入り口を統合。
- **一般ユーザー (User)**: マップの閲覧、現在地検索、混雑状況の草の根報告、口コミ投稿が可能。
- **店舗ホスト (Host)**: 地図上をクリックすることで「自分の店舗」を新規公式ピンとして登録可能。また、自店舗の混雑情報を公式として上書き更新可能（口コミ投稿は不可）。

### ブランチのルール
機能追加：app-あだ名(自由)-数字
機能修正：fix-あだ名(自由)-数字
デザイン：design-あだ名(自由)-数字
→他のブランチ名を使う場合も同じようにやってください
→ブランチ名の追加をした場合は追加をして下さい